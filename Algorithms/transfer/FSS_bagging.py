...
class FSS_bagging(object):
    ...
    def run(self, Xsource, Xtarget, Ysource, Ytarget, loc, MultiObj):
        prediction = dict()
        ...
        if MultiObj == False:
            auc = [roc_auc_score(Ytarget, res)]
            return auc
        else:
            if -1 in Ytarget and 0 in res:
                Ytarget[Ytarget == -1] = 0
            elif 0 in Ytarget and -1 in res:
                res[res == -1] = 0
            ...
            try:
                all_tres[0] = roc_auc_score(Ytarget, res)
            except Exception:
                pass

            try:
                all_tres[1] = f1_score(Ytarget, res)
            except Exception:
                pass

            try:
                all_tres[2] = accuracy_score(Ytarget, res)
            except Exception:
                pass

            try:
                all_tres[3] = recall_score(Ytarget, res)
            except Exception:
                pass
            ...
            try:
                all_tres[5] = precision_score(Ytarget, res)
            except Exception:
                pass

            try:
                all_tres[6] = matthews_corrcoef(Ytarget, res)
            except Exception:
                pass
