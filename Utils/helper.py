...
def MfindCommonMetric(list, ftarget, split=False):
    ...
    if len(common) == 0:
        ...
        # Instead of using pop, iterate through flist directly
        sx, sy, Stype = GetData(flist[0], showType=True)  # Get first project data
        fsx = np.zeros((len(sy), len(common)))
        for i in range(len(common)):
            index = Stype.index(common[i])
            fsx[:, i] = sx[:, index]

        # Initialize sy as a copy to keep it intact
        sy = np.copy(sy)

        loc = []
        base = 0

        # Iterate through the rest of flist, avoiding using pop
        for item in flist[1:]:  # Skip the first project since it's already processed
            x, y, Type = GetData(item, showType=True)
            loc.append(base)
            base += len(y)

            # Initialize fx for the current project
            fx = np.zeros((len(y), len(common)))
            for i in range(len(common)):
                index = Type.index(common[i])
                fx[:, i] = x[:, index]

            # Concatenate the current project's data to fsx and sy
                ...
                try:
                    ...
                    for item in trails.trials:
                        ...
                        if item['state'] == 2:
                            ...
                            results.append(item['result1']['result1'])
                            his[j] = results
                            j += 1
                    if j > 0:
                        inc_value = trails.best_trial['result1']['result1']
